...
## script in res://tests/ or addons/godot_mcp_server/testing/suites/.
##
## Source: ported from godot-ai (test_suite.gd).
## Enhanced variant: replaces McpScenePath with CommandHelpers.find_node;
## drops McpLogBuffer dependency (enhanced has no ring-buffer logger).
## API is otherwise byte-compatible with godot-ai's McpTestSuite so AI
## that learned the godot-ai docs can write suites against enhanced.

const CommandHelpers := preload("res://addons/godot_mcp_server/commands/command_helpers.gd")
...
## Enhanced variant: uses CommandHelpers.find_node instead of McpScenePath.
...
func _remove_control(path: String) -> void:
	...
	if scene_root == null:
		return
	var node := CommandHelpers.find_node(scene_root, path)
	if node != null:
