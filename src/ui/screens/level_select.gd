...
func _ready() -> void:
	...
	$Layout/Header/Operator.text = "OPERATOR: %s" % operative["name"]
	$Layout/Header/Operator.add_theme_color_override("font_color", operative["color"])
	$Layout/Footer/Samples.text = "ALIEN SAMPLES: %d" % int(SaveManager.profile.get("total_crystals", 0))
...
func _configure_mission(level_id: String) -> void:
	...
	item.text = "%02d  %s\n%s     %s" % [LevelCatalog.get_level_number(level_id), data["name"], data["subtitle"], "BEST %d" % best if completed else data["location"]]
	item.disabled = not unlocked
	item.add_theme_color_override("font_color", data["accent"])
	if not unlocked:
		item.text = "%02d  CLASSIFIED\nComplete the previous mission to unlock" % LevelCatalog.get_level_number(level_id)
