...
Reference ภาพรวมแคมเปญ `ARTREF-CAMPAIGN-ANCHOR` อยู่สถานะ Review ที่ `art_source/references/generated_campaign_style_anchor.png` ใช้กำหนด palette, depth, landmark, tile silhouette และภาษาชีวภาพต่างดาวของห้า biome เท่านั้น ห้ามใช้เป็น runtime asset และมี generation record อยู่ข้างไฟล์ภาพ
...
### สรุปการตรวจ generated source

[ASSET-AUDIT-01] การตรวจวันที่ 2026-08-30 พบ PNG candidate 474/474 ไฟล์เปิดได้ แต่ไฟล์ `_normalized_` ทั้ง 188 ไฟล์มี alpha กึ่งโปร่งใสในสัดส่วนมาก ความสมบูรณ์ระดับไฟล์ผ่าน แต่การอนุมัติขอบคมแบบ pixel art, grid ราย package, runtime filtering, memory บนแพลตฟอร์ม และ human visual review ยังติด blocker หรือรอตรวจ คำว่า `true alpha` หมายถึงมี transparency เท่านั้น ไม่ใช่การรับรองขอบสะอาด ไม่มี generated candidate ใดเลื่อนเกิน `Review` จากผลนี้ ดู [รายงานตรวจ Generated Asset](GENERATED_ASSET_AUDIT_TH.md)
...
|---|---|---|---|
| ENEMY-THORNLING | เงาะ | body true-alpha แบบละเอียด 6 action และ contact-hit VFX เงาะแยก 4 เฟรมอยู่ใน Review ที่ `art_source/generated/enemies/thornling/`; body ใช้ cell 700 x 800 baseline 740 และ VFX ใช้ cell 700 x 700 ที่ขอบสะอาดพร้อม hash/provenance; runtime ยังใช้ static idle เก่าและยังขาด integration/gameplay validation | Review |
| ENEMY-SPITTER | มะกรูด | body true-alpha แบบละเอียด 7 action และ seed/glob/impact VFX มะกรูดแยก 4 ชุดอยู่ใน Review ที่ `art_source/generated/enemies/spitter/`; body ใช้ cell 700 x 800 baseline 740 ส่วน VFX ใช้ cell จัตุรัส 700/800 px ที่ขอบสะอาด มี silhouette, hash และ provenance แยกชัดเจน; runtime ยังใช้ static idle เก่าและยังขาด integration/gameplay validation | Review |
| ENEMY-MAW | มังคุดอ่อน | มี source set แบบละเอียด true-alpha 6 action ที่ `art_source/generated/enemies/maw/`; normalized 4 x 1, cell 700 x 800, baseline 740; รอ fruit-read review และ integration | Review |
| ENEMY-ROOT-SKITTER | สละ | มี source set แบบละเอียด true-alpha 7 action ที่ `art_source/generated/enemies/root_skitter/`; normalized 4 x 1, cell 700 x 800, baseline 740; รอ runtime integration | Review |
| ENEMY-EYE-WISP | ลำไย | body true-alpha แบบละเอียด 7 action และ seed-bolt/impact/beam VFX ลำไยแยก 4 ชุดอยู่ใน Review ที่ `art_source/generated/enemies/eye_wisp/`; body ใช้ cell 700 x 800 และ lower visual guide 740 ส่วน VFX ใช้ cell 700 x 700 ที่ขอบสะอาด มี silhouette คงที่, hash และ provenance ครบ; ยังขาด runtime integration/gameplay validation | Review |
| ENEMY-CAPSULE-HUSK | กระท้อน | มี source set แบบละเอียด true-alpha 7 action ที่ `art_source/generated/enemies/capsule_husk/`; normalized 4 x 1, cell 700 x 800, baseline 740; รอ runtime integration | Review |
...
|---|---:|---|---|---|
| BOSS-THORN-MATRIARCH | 1 | พวงราชินีเงาะ | ชุดตัวบอส true-alpha 9 action พร้อม mine/burst, hair-thorn/impact, lane hazard ที่มี tell, portrait 3 สถานะ, intro/HUD frame และ phase marker อยู่ใน Review ที่ `art_source/generated/bosses/thorn_matriarch/`; ยังต้อง integrate runtime, ทำ VFX/SFX ที่เหลือ และตรวจ gameplay | Review |
| BOSS-MAW-SOVEREIGN | 2 | มงกุฎทุเรียน + กายมังคุด | body true-alpha แบบละเอียด 12 action, projectile/hazard แยก 5 ชุด, portrait 3 สถานะ, intro/HUD frame และ phase marker อยู่ใน Review ที่ `art_source/generated/bosses/maw_sovereign/`; บันทึก fruit identity sheet, normalized grids, hashes และ provenance แล้ว; ยังขาด runtime integration, VFX/SFX ขั้นสุดท้าย และ gameplay validation | Review |
| BOSS-POSSESSED-BANYAN | 3 | ขนุน + ลูกไทร | body true-alpha แบบละเอียด 13 action, projectile/hazard/death-VFX แยก 6 ชุด และ presentation แบบไม่มีข้อความอยู่ใน Review ที่ `art_source/generated/bosses/possessed_banyan/`; presentation มี portrait 3 สถานะ, intro frame, compact HUD frame และ phase marker 3 แบบ พร้อมตรวจ alpha opening แล้ว; บันทึก identity sheet, normalized grids, hashes และ provenance ครบ; ยังขาด runtime integration, VFX/SFX ขั้นสุดท้าย และ gameplay validation | Review |
| BOSS-ROOT-HYDRA | 4 | พวงลูกจาก | fruit identity, body true-alpha 11 action/44 เฟรม, projectile/telegraph/impact แยก 7 ชุด และ presentation แบบไม่มีข้อความครบอยู่ใน Review ที่ `art_source/generated/bosses/root_hydra/`; portrait, intro/HUD frame และ phase marker ปิด source contract แบบ Touhou จาก 4 จุดยิง; ยังขาด runtime integration, VFX/SFX ขั้นสุดท้าย และ gameplay validation | Review |
| BOSS-ROOT-CORE-EYE | 5 | พวงตาลำไย + กลีบแก้วมังกร | fruit identity, body true-alpha 13 action/52 เฟรม, projectile/telegraph/impact แยก 7 ชุด และ presentation แบบไม่มีข้อความครบอยู่ใน Review ที่ `art_source/generated/bosses/root_core_eye/`; portrait, intro/HUD frame และ phase marker ปิด source contract ของจุดยิงตายอด 5 จุดร่วมกับตากลาง พร้อม normalized grid, hash และ provenance; ยังขาด runtime integration, VFX/SFX ขั้นสุดท้าย และ gameplay validation | Review |
...
| WORLD-L2-FOREST | ชุด source pixel art รายละเอียดสูงอยู่ใน Review ที่ `art_source/generated/world/level_02_mutated_forest/` มี parallax 5 ชั้น, tile source แบบ modular 7 ชิ้นพร้อมข้อกำหนด variant mirror/rotate, hazard spore vent มังคุดแบบ animation, extraction beacon เฉพาะ biome, Maw Bloom landmark และ prop ศาลภาคสนาม; บันทึก alpha/hash/provenance แล้ว แต่ยังขาด runtime slicing, collision, placement, memory และ gameplay validation ที่ 1280 × 720 | Review |
| WORLD-L3-CAPSULE | ชุด source pixel art รายละเอียดสูงอยู่ใน Review ที่ `art_source/generated/world/level_03_capsule_07/` มี parallax ถ้ำ/เปลือก 5 ชั้น, tile source capsule-root แบบ modular 7 ชิ้นพร้อมข้อกำหนด variant mirror/rotate, hazard seed piston กระท้อนแบบ animation, extraction beacon ห้องราก และ Capsule 07 seed-harvester landmark; บันทึก alpha/hash/provenance แล้ว แต่ยังขาด runtime slicing, collision, placement, memory และ gameplay validation ที่ 1280 × 720 | Review |
| WORLD-L4-MARSH | Devouring Root Marsh | ชุด v2 candidate อยู่ใน Review ที่ `art_source/generated/world/level_04_root_marsh/` มีชั้นหนองน้ำ/nipa/conduit 5 ชั้น, root-mat 7 ชิ้น, nutrient-root eruption 4 เฟรม, extraction beacon และ conduit landmark; record ระบุ square pixel, ขอบขั้นบันได, limited ramp, gutter, hash และการ reject v1 แต่ global audit พบ soft alpha จึงยังไม่ผ่าน independent pixel-art approval และยังขาด runtime slicing, collision, filtering, placement, memory และ validation 1280 × 720 | Review |
| WORLD-L5-NEXUS | Alien Eye Nexus | ชุด v2 candidate อยู่ใน Review ที่ `art_source/generated/world/level_05_alien_eye_nexus/` มีชั้น impossible-depth/ตาลำไย/กลีบแก้วมังกร 5 ชั้น, source membrane/neural-root 7 ชิ้น, sensory platform 4 เฟรม, extraction beacon และ nexus landmark; record ระบุ square-pixel construction, ขอบขั้นบันได, limited ramp, gutter, hash และการ reject v1 แต่ global audit พบ soft alpha จึงยังไม่ผ่าน independent pixel-art approval และยังขาด runtime slicing, collision, filtering, placement, projectile readability, memory และ validation 1280 × 720 | Review |
| LANDMARK-CAPSULE-07 | source true-alpha ของ Capsule 07 แบบ seed harvester อยู่ใน Review ที่ `art_source/generated/world/level_03_capsule_07/landmarks/capsule_07_seed_harvester_v1.png` แสดง germination core ทรงกระท้อน, silhouette เปลือก/นั่งร้าน และรากดูดทรัพยากรโดยไม่มีข้อความ baked; ยังขาด runtime scale, placement, occlusion และ validation ของ midpoint reveal | Review |
| LANDMARK-ROOT-CONDUIT | nutrient conduit | pixel-art source candidate `art_source/generated/world/level_04_root_marsh/landmarks/root_nutrient_conduit_v2.png` บันทึก scale ประตูชลประทาน ทางผ่านด้านล่าง บริบทสละ/nipa hash และการ reject v1 แล้ว แต่ยังขาด hard-edge approval, runtime placement, collision/readability และ performance validation | Review |
| LANDMARK-ALIEN-EYE | sensory nexus | pixel-art source candidate `art_source/generated/world/level_05_alien_eye_nexus/landmarks/awakened_sensory_nexus_v2.png` บันทึกตาเมล็ดลำไย ม่านตากลีบแก้วมังกร สาย biomass ทางผ่านด้านล่าง hash และการ reject v1 แล้ว แต่ยังขาด hard-edge approval, runtime scale, placement, occlusion, projectile readability และ performance validation | Review |
...
| UI-DIALOGUE-FRAME | source true-alpha ไม่มีข้อความ 2300 × 800 px อยู่ใน Review ที่ `art_source/generated/ui/narrative/`; ยังขาด runtime slicing/portrait/ข้อความสองภาษา | Review |
| UI-RADIO-OVERLAY | runtime มีโหมด compact ด้านขวาบนแล้ว และกรอบ pixel art true-alpha ไม่มีข้อความอยู่ใน Review ที่ `art_source/generated/ui/narrative/`; ยังขาด skin replacement/ข้อความสองภาษา | Review |
...
| UI-BOSS-HUD | frame, portrait state และ phase marker แบบไม่มีข้อความของบอสครบทั้ง 5 ตัวอยู่ใน Review ใต้ `art_source/generated/bosses/*/presentation/`; ยังต้องทำ reusable runtime component, ตรวจข้อความสองภาษา และ gameplay-scale validation | Review |
...
| UI-MASTERY | mastery screen true-alpha ไม่มีข้อความ 1800 × 1000 px พร้อม rank node 6 สถานะ cell 750 px อยู่ใน Review ที่ `art_source/generated/ui/mastery/`; ยังขาด runtime binding/state/ข้อความสองภาษา | Review |
| VFX-CUTTER-SET | source true-alpha แยก 4 ชุดสำหรับ swing, organic contact, charged swing และ upgrade activation อยู่ใน Review ที่ `art_source/generated/vfx/cutter/`; runtime ยังใช้ effect เก่าจนกว่าจะ integrate timing/collision | Review |
| VFX-DAMAGE-SET | source true-alpha แยก 5 ชุดสำหรับ player hit, organic hit, armored hit, final-boss-core hit และ loop สถานะ root contamination อยู่ใน Review ที่ `art_source/generated/vfx/damage/`; ทุกชุดใช้ cell 800 px ขอบสะอาดพร้อม hash/provenance แต่ยังขาด runtime event binding, scale, timing, blend mode และ gameplay validation | Review |
