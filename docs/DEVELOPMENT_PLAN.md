...
**Boss-asset promotion evidence — 2026-08-31:** The Root Hydra (nipa-palm cluster) now has a matching armored-idle runtime pilot. Its authored strip is quantized to 4 × 1000 × 900 cells with an 840 px baseline, nearest filtering and 3 fps breathing loop; the existing multi-origin projectile contract is unchanged. Smoke coverage checks the promoted texture, four-frame grid and -105 px baseline offset. The package remains `Integrated` pending phase-specific body animation, projectile/presentation art, audio and human boss-feel review.
